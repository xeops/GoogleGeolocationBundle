...
class GeolocationApi
{
	 ...
	 * @var string
	 */
	private $url = 'maps.googleapis.com/maps/api/geocode/';

	private $format = 'json';
	/**
	 * Protocol for Google API queries
	 * @var string
	 */
	private $proto = 'http';

	/**
	 * @see https://developers.google.com/maps/faq?hl=ru#languagesupport
	 * @var string
	 */
	 ...
	 * After setting api key, request will prefer https
	 * @param string $apiKey
	 * @return GeolocationApi
	 */
	public function setApiKey(string $apiKey): GeolocationApi
	{
		$this->apiKey = $apiKey;
		$this->proto = $this->apiKey === null ? 'http' : 'https';
		return $this;
	}
	...
	public function locateAddress($search, $searchType = 'address')
	{
		$location = null;
		...
		if (true === is_null($location))
		{
			...
			$location = $this->geolocate($location, $searchType);
		}
		else
		{
	 ...
	 * @param $lat
	 * @param $lng
	 * @return Location|Google\GeolocationBundle\Entity\Location|mixed|null
	 */
	public function locateByCoordinates($lat, $lng)
	{
		$search = "$lat,$lng";

		return $this->locateAddress($search, 'latlng');
	}
	 ...
	 */
	protected function geolocate(\Google\GeolocationBundle\Entity\Location $location, $searchAtribute = 'address')
	{
		// Check limiting
		if ($this->apiAttemptsAllowed())
		{
			$response   = $this->request(array($searchAtribute => $location->getSearch()));
	...
	protected function request($params)
	{
		...
		return $this->browser->get($this->proto . '://' . $this->url . $this->format . "?" . http_build_query( $params));
	}
	 ...
	 * @return GeolocationApi
	 */
	public function setLanguage(string $language): GeolocationApi
	{
	 ...
	 * @return GeolocationApi
	 */
	public function setFormat(string $format): GeolocationApi
	{
		$this->format = $format;
		return $this;
	}
